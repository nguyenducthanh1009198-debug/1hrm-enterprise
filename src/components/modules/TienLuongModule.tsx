      ...
      {!isHR && (
        <div className="p-3.5 bg-amber-50 border border-amber-200 rounded-xl flex items-center gap-3 text-xs text-amber-900">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="p-4 bg-gradient-to-br from-orange-50 to-amber-50 rounded-xl border border-orange-200 shadow-xs flex flex-col justify-between">
              <div>
                <p className="text-xs text-orange-900 font-bold">Tính Toán Tự Động Bằng Formula Engine</p>
                <p className="text-[11px] text-orange-700 mt-0.5">Nạp dữ liệu công + KPI và chạy công thức</p>
              </div>
              <button
                onClick={handleRunRecalculate}
                className="mt-2 flex items-center justify-center gap-1.5 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-xs font-bold shadow-md shadow-orange-600/20 transition-all cursor-pointer"
              >
                        ...
                        <td className="py-3 px-4 text-right">
                          {canView ? (
                            <button
                              ...
                              className="px-3 py-1 bg-orange-50 hover:bg-orange-100 text-orange-600 rounded-lg font-semibold transition-colors inline-flex items-center gap-1 cursor-pointer"
                            >
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                className="w-full p-3 font-mono text-xs bg-slate-900 text-amber-400 rounded-xl focus:ring-2 focus:ring-orange-500 focus:outline-none"
              />
            </div>
            ...
            <div className="flex items-center justify-between">
              <button
                ...
                className="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-xs font-bold shadow-md shadow-orange-600/20"
              >
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
              {SYSTEM_FORMULA_PRESETS.map((f, i) => (
                <div key={i} className="p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="font-mono font-bold text-orange-600">{f.code}</span>
                  ...
                  <p className="font-mono text-[11px] text-slate-600 bg-white p-1.5 rounded border border-slate-100">
                    {f.expression}
                  </p>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            ...
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
              ...
              <p className="font-black text-orange-600 text-lg mt-0.5">
                {totalCompanyPayroll.toLocaleString('vi-VN')} đ
              </p>
      ...
      {selectedPayslip && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            ...
            <div className="p-4 bg-orange-50/70 rounded-xl border border-orange-200 flex items-center justify-between">
              <div>
                <p className="text-xs text-orange-900 font-bold">{selectedPayslip.employeeName}</p>
                <p className="text-[11px] text-orange-700">{selectedPayslip.departmentName} • {selectedPayslip.employeeCode}</p>
              </div>
              <div className="text-right">
                <span className="text-[10px] uppercase font-bold text-orange-800">Thực Nhận</span>
                <p className="text-xl font-black text-orange-600">{selectedPayslip.netSalary.toLocaleString('vi-VN')} đ</p>
              </div>
            </div>
